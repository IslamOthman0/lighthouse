...
async function syncSingleMember(member, todayTimeEntries, avgTasksBaseline = 3, settings = null, runningEntry = null, globalTaskCache = {}, workingDays = 1, startDate = null, endDate = null) {
  try {
    ...
    if (member.id && startDate && endDate) {
      try {
        logger.debug(`${member.name}: querying db.leaves for memberId=${member.id} (type=${typeof member.id}), range=[${startDate.toISOString()} - ${endDate.toISOString()}]`);
        const memberLeaves = await db.leaves.where('memberId').equals(member.id).toArray();
        logger.debug(`${member.name}: found ${memberLeaves.length} leave record(s). Types: [${memberLeaves.map(l => l.type).join(', ')}]. Statuses: [${memberLeaves.map(l => l.status).join(', ')}]`);
